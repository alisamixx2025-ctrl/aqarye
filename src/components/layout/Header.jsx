...
import { handlePackageCheck, truncate, isRTL } from "@/utils/helperFunction";
                  ...
                  {showLangDropdown && (
                    ...
                    >
                      {languages.map((lang) => (
                        ...
                        >
                          <span className={`transition-all duration-150 ${isRtl ? 'group-hover:mr-2' : 'group-hover:ml-2'}`}>
                            {lang.name}
                          </span>
                        </div>
                      ))}
              ...
              <div className="flex items-center gap-6">
                <Link href={`/?lang=${lang || "en"}`} title="Home">
                  ...
                  <ImageWithPlaceholder
                    ...
                    width={176}
                    height={56}
                    className="w-44 h-14 aspect-[176/56] object-cover"
                    priority={true}
                  />
                  {/* </div> */}
                </Link>
                ...
                <ul className="hidden items-center gap-4 xl:flex">
                  ...
                  {menus.map((menu) => (
                    <li key={menu.name} className="relative dropdown-menu flex flex-col items-center">
                      ...
                      {openMenu === menu.name && (
                        <div className={`dropdown-content absolute ${isRtl ? 'right-0' : 'left-0'} top-full z-20 mt-2 w-[280px] rounded-md cardBg shadow-lg newBorder`}>
                          <ul className="py-1 [&>li:last-child>button]:border-b-0">
                            {menu.links.map((link) => (
                              <li key={link.name}>
                                <button
                                  className={`hover:primaryColor hover:primaryBorderColor group block w-full cursor-pointer border-b-2 border-dashed px-3 py-2 ${isRtl ? 'text-right' : 'text-left'} transition-all duration-150`}
                                  onClick={(e) => {
                                    e.preventDefault();
                                    e.stopPropagation();
                      ...
                      {openMenu === "user" && (
                        <div className="dropdown-content absolute right-0 top-full z-20 mt-2 min-w-[130px] rounded-md bg-white shadow-lg border">
                          <ul className="py-1">
                            ...
                            <li>
                              <button
                                className={`hover:primaryColor hover:primaryBorderColor group block w-full cursor-pointer px-3 py-2 ${isRtl ? 'text-right' : 'text-left'} transition-all duration-150`}
                                onClick={(e) => {
                                  e.preventDefault();
                                  e.stopPropagation();
                                  handleLogout();
