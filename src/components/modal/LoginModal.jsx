...
const LoginModal = ({ showLogin, setShowLogin }) => {
  ...
  const isGreenApiOtp = settingsData?.otp_service_provider === "greenapi";
  ...
  const handlePhoneForgotPassword = async () => {
    ...
    try {
      ...
      if (isGreenApiOtp) {
        await generateOTPWithGreenAPI(rawPhoneNumber);
      } else if (isTwilioOtp) {
        await generateOTPWithTwilio(rawPhoneNumber);
      } else {
        // Default/fallback to Firebase if configured or if Twilio/GreenApiOtp not enabled
  ...
  // GreenAPI OTP generation (used for GreenAPI)
  const generateOTPWithGreenAPI = async (phoneNumber) => {
    setShowLoader(true);
    ...
    try {
      ...
      console.error("Error generating OTP with GreenAPI:", error);
    ...
    try {
      ...
      if (error?.data?.user_exists === true && error?.data?.password_exists === false) {
        try {
          ...
          } else if (isGreenApiOtp) {
            await generateOTPWithGreenAPI(fullPhoneNumberWithCountryCode);
          }
  ...
  const handleConfirm = (e) => {
    ...
    if (isFirebaseOtp) {
      ...
      try {
        handleGreenAPIOTPConfirm();
      } catch (error) {
        console.error("Error verifying OTP with GreenAPI:", error);
        toast.error(t(error.message) || t("otpVerificationFailed"));
        setShowLoader(false);
      }
    }
  ...
  const handleGreenAPIOTPConfirm = async () => {
    try {
      ...
      if (isEmailOtpEnabled) {
        ...
        if (isFirebaseOtp) {
          ...
          try {
            ...
            console.error("GreenAPI OTP error:", error);
            toast.error(t(error?.message) || t("failedToSendOTP"));
            setIsCounting(false);
            setTimeLeft(0);
            setShowLoader(false);
  ...
  const sendPhoneRegistrationOTP = async (fullPhoneNumber) => {
    if (isFirebaseOtp) {
      ...
      await generateOTPWithGreenAPI(fullPhoneNumber);
    }
  };
